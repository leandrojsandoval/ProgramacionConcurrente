def download_sprite(url, character_name, type_sprite):
    ...
    if not os.path.exists(sprite_path):
        ...
        if response.status_code == 200:
            ...
            print(
                f"Sprite {type_sprite} de {character_name} guardado en {sprite_path}"
            )
        else:
            print(
        ...
        print(
            #f"Sprite {type_sprite} de {character_name} ya existe en {sprite_path}."
        )


def download_and_save_character_data(character_count, api_url,
                                     path_folder_characters):
    ...
    for character_id in range(1, character_count + 1):
        ...
        if response.status_code == 200:
            ...
            for type_sprite, url_sprite in character_data["sprites"].items():
                if url_sprite:  # Solo descargar si la URL es válida
                    download_sprite(url_sprite, character_name, type_sprite)

            # Extraer los datos relevantes para las estadísticas
            hp = character_data["stats"][0]["base_stat"] if len(
                character_data["stats"]) > 0 else 0  # HP
            attack = character_data["stats"][1]["base_stat"] if len(
                character_data["stats"]) > 1 else 0  # Ataque
            defense = character_data["stats"][2]["base_stat"] if len(
                character_data["stats"]) > 2 else 0  # Defensa
            ...
            character_stats = {
                ...
                defense,
                "front_sprite_path":
                f"sprites/{character_name.lower()}_front_default.png",
                "back_sprite_path":
                f"sprites/{character_name.lower()}_back_default.png"
            }

            # Guardar los datos en un archivo JSON
            output_file = os.path.join(path_folder_characters,
                                       f"{character_name.capitalize()}.json")
            with open(output_file, "w") as f:
                json.dump(character_stats, f, indent=4)
            print(
                f"Datos del personaje {character_name} guardados en {output_file}"
            )
